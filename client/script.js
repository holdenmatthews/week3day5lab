...
const helloBtnChange = () => {
    sayHelloButton.style.backgroundColor = "black"
    sayHelloButton.style.color = "white"
}

sayHelloButton.addEventListener("mouseover", helloBtnChange)
...
const helloBtnReturn = () => {
    sayHelloButton.style.backgroundColor = "#EFEFEF"
    sayHelloButton.style.color = "black"
}

sayHelloButton.addEventListener("mouseout", helloBtnReturn)
...
/*
    ...
    pass a callback function to. Inside the callback function, console.log 
    the response's data (in the intermediate instructions we'll come back to 
...
const ohMy = () => {
    axios.get(`${baseURL}/animals`).then((res) => {
        for (let i = 0; i < res.data.length; i++) {
            ...
            para.textContent = res.data[i].value
...
/*
    ...
    below make get request to 'http://localhost:3000/repeat/{SOMEPARAM}', but with 
    a string instead of {SOMEPARAM}.  

    The function that runs when this request is made will return whatever parameter you sent 

    Handle the promise returned from the request with a .then, which will take in 
    a callback -- the callback function should print the response.data.
...
const repeatMyParam = () => {
    axios.get(`${baseURL}/repeat/"hello"`).then((res) => {
        console.log(res.data)
        const repeatTextBtn = document.querySelector("#repeat-text")
        repeatTextBtn.style.textContent = res.data
...
const sendQuery = () => {
    axios.get(`${baseURL}/query-test?name=holden`).then((res) => {
        console.log(res.data)
    }).catch((err) => {
        console.log(err)
    })
...
/* 
    ...
    textContent equal the string at the current index (i) and then append 
...
// PROBLEM 10 
/*
    In the function that you wrote for Problem 8, change the URL 
    to test a couple different scenarios. 
...
/*
    ...
    field should have a placeholder that tells the user to enter 
    ...
    (make sure to match the case and spelling exactly) and the 
    value should be the value of the food input. 
